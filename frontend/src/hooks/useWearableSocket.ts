...
export function useWearableSocket(onEvent:(event:SocketEvent)=>void) {
  ...
  useEffect(()=>{ let ws:WebSocket|undefined, cancelled=false, retry=0, timer:number|undefined;
    const connect=()=>{ if(cancelled)return; const socketUrl=import.meta.env.VITE_WS_URL; if(!socketUrl){console.error('VITE_WS_URL is not configured');setStatus('disconnected');return} setStatus(retry?'reconnecting':'disconnected'); ws=new WebSocket(socketUrl);
